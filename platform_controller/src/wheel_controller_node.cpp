...
*
* Description:
*	The wheel_controller_node
* 
***********************************************************************************/

#include "wheel_controller/wheel_controller_node.hpp"

/**\defgroup wheel_controller_node Wheel controller node
 * \relates WheelController
 * Main function of the wheel_controller node. Create a WheelController object and calls spinning code.
...
int main(int argc, char *argv[])
{
	...
	WheelController* wheel_controller = new WheelController("wheel_controller", n, serial_port, baudrate);
	if(!wheel_controller->enable())
	{
	...
	while(n.ok() && !stop)
	{
		...
		if(rose_conversions::kbhit() && test)
		{
		    ...
		    switch(c)
		    {
		    	case '+':	
		    		velocity += vel_step;    		
					for(auto& wheelunit : wheel_controller->getWheelUnits())
				...
				case 'b':
					...
					wheel_controller->setActiveBrakeMode(active_braking);	
					break;
				case 'x':
					stop = true;
					break;
		    ...
		    wheel_controller->writeWheelStates();
		}

		wheel_controller->update();

		//! @todo OH: move to wheel_controller.cpp ? Make generic logger?
		if(log_PID)
		{
			if(pid_log_drive != NULL && pid_log_steer != NULL)
			{
				...
				// Get debug states
				wheel_controller->getWheelUnitsDebugStates();
