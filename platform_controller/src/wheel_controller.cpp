...
//! @ todo:OH add braking support to wheel_controller

#include "wheel_controller/wheel_controller.hpp"
...
WheelController::WheelController(string name, ros::NodeHandle n, string serial_port, int baudrate)
    ...
    , velocity_watchdog_("wheel_controller_velocity_watchdog", n, VELOCITY_TIMEOUT, boost::bind(&WheelController::CB_cancelAllMovements, this))
{
    n_           	= n;
    name_       	= name;
    enabled_ 	 	= false;
...
bool WheelController::alarmStateOk()
{
    ...
    if(sh_platform_controller_alarm_)
    {
        ...
        ROS_ERROR_NAMED(ROS_NAME, "Alarm: %d | Alarm byte(%d): %s", alarm_number_, alarm_byte_, rose_conversions::byteToBinary(alarm_byte_));  //! @todo: OH should this be in the wheel controller class
...
bool WheelController::enable()
{
    ...
    if(!checkControllerID(WHEEL_CONTROL_FIRMWARE_ID)) 
        return false;

    if(!checkFirmwareVersion(WHEEL_CONTROL_FIRMWARE_MAJOR_VERSION, WHEEL_CONTROL_FIRMWARE_MINOR_VERSION))
        return false;

    ROS_INFO_NAMED(ROS_NAME, "Wheel controller with firmware version %d.%d connected.", received_firmware_major_version_, received_firmware_minor_version_);
    ...
    if(!setErrorTresholds(  WHEEL_CONTROLLER_FOLLOWING_ERR_TIMER,
                            WHEEL_CONTROLLER_CURRENT_ERR_TIMER, 
                            WHEEL_CONTROLLER_CONNECTION_ERR_TIMER,
                            WHEEL_CONTROLLER_MAE_ERR_TIMER))
    {
    ...
    if(!setDrivePIDs(   WHEEL_CONTROLLER_DRIVE_KI, 
                        WHEEL_CONTROLLER_DRIVE_K, 
                        WHEEL_CONTROLLER_DRIVE_KP, 
                        WHEEL_CONTROLLER_DRIVE_KD, 
                        WHEEL_CONTROLLER_DRIVE_ILIMIT, 
                        WHEEL_CONTROLLER_DRIVE_SCALE, 
                        WHEEL_CONTROLLER_DRIVE_SCALE, 
                        WHEELUNIT_MAX_VEL, 
                        WHEEL_CONTROLLER_DRIVE_FEMAX, 
                        WHEEL_CONTROLLER_DRIVE_MAX_CURR))
    {
    ...
    if(!setSteerPIDs(   WHEEL_CONTROLLER_STEER_KI, 
                        WHEEL_CONTROLLER_STEER_K, 
                        WHEEL_CONTROLLER_STEER_KP, 
                        WHEEL_CONTROLLER_STEER_KD, 
                        WHEEL_CONTROLLER_STEER_ILIMIT, 
                        WHEEL_CONTROLLER_STEER_SCALE, 
                        WHEEL_CONTROLLER_STEER_SCALE, 
                        STEER_VELOCITY_LIMIT*1000,                   //! @todo OH: UGLY magic number
                        WHEEL_CONTROLLER_STEER_FEMAX, 
                        WHEEL_CONTROLLER_STEER_MAX_CURR))
    {
    ...
    ControllerResponse 	response = *(new ControllerResponse(WHEEL_CONTROL_ENABLE, HARDWARE_CONTROL_TIMEOUT));
    response.addExpectedDataItem(ControllerData("1", enabled_, "Unable to enable low-level platform controller."));
    ControllerCommand 	command(WHEEL_CONTROL_ENABLE, response);
...
bool WheelController::disable()
{
    if(enabled_)
    {
        ...
        ControllerResponse 	response = *(new ControllerResponse(WHEEL_CONTROL_ENABLE, HARDWARE_CONTROL_TIMEOUT));
        response.addExpectedDataItem(ControllerData("0", enabled_, "Unable to disable low-level wheelunit controller"));
        ControllerCommand 	command(WHEEL_CONTROL_ENABLE, response);
...
{
    return simpleCommand(WHEEL_CONTROL_RESET_PLATFORM, HARDWARE_CONTROL_TIMEOUT);
}
...
bool WheelController::resetLowLevelSafety()
{
    ...
    alarm_number_                   = 0;
    alarm_byte_                     = 0;
    return simpleCommand(WHEEL_CONTROL_RESET_ALARM, HARDWARE_CONTROL_TIMEOUT);
}

bool WheelController::resetLowLevelCommunication()
{
    return simpleCommand(WHEEL_CONTROL_RESET_COMM, HARDWARE_CONTROL_TIMEOUT);
}
									...
									int MaxCurr)
{
    ...
    ControllerResponse response(WHEEL_CONTROL_DRIVE_PID_VALS, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand  command(WHEEL_CONTROL_DRIVE_PID_VALS, response);
									...
									int MaxCurr)
{
    ...
    ControllerResponse response(WHEEL_CONTROL_STEER_PID_VALS, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand command(WHEEL_CONTROL_STEER_PID_VALS, response);
    command.addDataItem(Ki);
    command.addDataItem(K);
    command.addDataItem(Kp);
    command.addDataItem(Kd);
...
{
    ControllerResponse response(WHEEL_CONTROL_ERROR_TIMERS, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand  command(WHEEL_CONTROL_ERROR_TIMERS, response);
...
bool WheelController::setStartStopValues(int start_value_low_level, int stop_value_low_level)
{
    ...
    ControllerResponse response(WHEEL_CONTROL_START_STOP_VALS, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand  command(WHEEL_CONTROL_START_STOP_VALS, response);
...
bool WheelController::setActiveBrakeMode(bool active_brake)
{
    ...
    ControllerResponse response(WHEEL_CONTROL_BRAKE_MODE, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand  command(WHEEL_CONTROL_BRAKE_MODE, response);
...
bool WheelController::getWheelUnitDrivePosition(WheelUnit& wheelunit)
{
    ...
    // Drive motor
    // Get new position
    response = new ControllerResponse(WHEEL_CONTROL_GET_POSITION, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.measured_drive_encoder_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_POSITION, *response);
...
{    
    ...
    response = new ControllerResponse(WHEEL_CONTROL_GET_ALL_DRIVE_ENC_DIFF, HARDWARE_CONTROL_TIMEOUT);
    ...
    command  = new ControllerCommand(WHEEL_CONTROL_GET_ALL_DRIVE_ENC_DIFF, *response);

    if(!executeCommand(*command))
        return false;
    
    float dT = (float)clock_diff_CNT/(float)WHEEL_CONTROLLER_CLK_FREQ;
...
bool WheelController::getWheelUnitSteerVelocity(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_VELOCITY, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.encoder_measured_velocity_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_VELOCITY, *response);
...
bool WheelController::getWheelUnitVelocityError(WheelUnit& wheelunit)
{
	...
	// Drive motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_VEL_ERROR, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.velocity_error_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_VEL_ERROR, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitPositionError(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_POS_ERROR, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.position_error_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_POS_ERROR, *response);
    command->addDataItem(wheelunit.getSteerMotorID());
...
bool WheelController::getWheelUnitPIDOut(WheelUnit& wheelunit)
{
    ...
    // Drive motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_PIDOUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.drive_PID_out_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_PIDOUT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitPOut(WheelUnit& wheelunit)
{
    ...
    // Drive motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_POUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.drive_P_out_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_POUT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitIOut(WheelUnit& wheelunit)
{
    ...
    // Drive motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_IOUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.drive_I_out_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_IOUT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitDOut(WheelUnit& wheelunit)
{
    ...
    // Drive motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_DOUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.drive_D_out_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_DOUT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitMAE(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_MAE_OUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.MAE_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_MAE_OUT, *response);
    command->addDataItem(wheelunit.getSteerMotorID());
...
bool WheelController::getWheelUnitFE(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_FE_OUT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.FE_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_FE_OUT, *response);
    command->addDataItem(wheelunit.getSteerMotorID());
...
bool WheelController::getWheelUnitDriveCurrent(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_CURRENT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.measured_drive_current_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_CURRENT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitMaxDriveCurrent(WheelUnit& wheelunit)
{
    ...
    // Steer motor
    response = new ControllerResponse(WHEEL_CONTROL_GET_MAX_CURRENT, HARDWARE_CONTROL_TIMEOUT);
    response->addExpectedDataItem(ControllerData(wheelunit.measured_max_drive_current_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_MAX_CURRENT, *response);
    command->addDataItem(wheelunit.getDriveMotorID());
...
bool WheelController::getWheelUnitsDebugStates()
{
    ...
    ControllerResponse response(WHEEL_CONTROL_GET_DEBUG_STATUS, HARDWARE_CONTROL_TIMEOUT);
    ...
    ControllerCommand command(WHEEL_CONTROL_GET_DEBUG_STATUS, response);
...
bool WheelController::getAlarmStatus()
{
    ...
    response = new ControllerResponse(WHEEL_CONTROL_GET_ALARM_STATE, HARDWARE_CONTROL_TIMEOUT);
    ...
    command  = new ControllerCommand(WHEEL_CONTROL_GET_ALARM_STATE, *response);
...
bool WheelController::getStatus()
{
    ...
    response = new ControllerResponse(WHEEL_CONTROL_GET_COMPLETE_STATUS, HARDWARE_CONTROL_TIMEOUT);
    ...
    response->addExpectedDataItem(ControllerData(alarm_number_));
    response->addExpectedDataItem(ControllerData(alarm_byte_));
    command  = new ControllerCommand(WHEEL_CONTROL_GET_COMPLETE_STATUS, *response);
    ...
    float dT = (float)clock_diff_CNT/(float)WHEEL_CONTROLLER_CLK_FREQ;              //! @todo OH: Duplicate code with getAllWheelUnitDriveEncoderSpeeds
...
{
    WheelUnit wheelunit;
    ControllerResponse  response = *new ControllerResponse(WHEEL_CONTROL_MOVE, HARDWARE_CONTROL_TIMEOUT);
    ControllerCommand   command(WHEEL_CONTROL_MOVE);
